...
int check_open(Coord goal, Node** op_list, int size){
    if(size > 0) // Check if Open list has elements in it
    {
        int index_of_min = 0;
        int min_dist = calc_dist(op_list[0]->pos, goal);
        for(int i = 0; i < size; i++)
        {
            Coord current = op_list[i]->pos;
            if(calc_dist(current, goal) < min_dist)
            {
                min_dist = calc_dist(current, goal);
                index_of_min = i;
            }
        }
        return index_of_min;
...
void main(){
    ...
    while(success != 1){
        ...
        open_index = check_open(goal, open_list, open_size); // Get index of open node with the best heuristic value
